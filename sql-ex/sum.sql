SELECT i,
	...
	END+' '+
  +CASE
		WHEN (((LEN(convert(varchar(20), iprice))-i) % 3)=2) OR (((LEN(convert(varchar(20), iprice))-i) % 3)=0) AND (i=1)
    THEN
         CASE (SUBSTRING (CONVERT (VARCHAR (20), iprice), i+1, 1))
            WHEN '0' THEN ''
            WHEN '1' THEN 'one'
       ...
       + CASE
            ...
            THEN
               'zero'
            WHEN     ( (SUBSTRING (CONVERT (VARCHAR (20), iprice), i, 1)) <>
                         '0')
                 AND ( ( (LEN (CONVERT (VARCHAR (20), iprice)) - i) % 3) = 2)
            THEN
               'hundred '
            ELSE
               ''
       ...
       + CASE (SUBSTRING (CONVERT (VARCHAR (20), iprice), i+1, 1))
         ...
         END
       + ' '
       + CASE (SUBSTRING(convert(varchar(20), iprice),i+2,1))
	WHEN '0' THEN 'ten'
	WHEN '1' THEN 'eleven'
	WHEN '2' THEN 'twelve'
	WHEN '3' THEN 'thirteen'
